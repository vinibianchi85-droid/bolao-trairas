...
function telefoneParaUsuario(telefone) {
  return (telefone || '').replace(/\D/g, '')
}

function usuarioToAuthEmail(usuario) {
  const clean = limparUsuario(usuario)
  if (!clean) return ''
...
function App() {
  ...
  async function signUp() {
    ...
    const username = limparUsuario(nome.split(' ')[0] + whats.slice(-4))
    const authEmail = usuarioToAuthEmail(username)

    if (!nome || !whats || !email || !senha) {
      return setMsg('Preencha nome, telefone, e-mail e senha.')
    }

    if (!username || username.length < 8) {
      return setMsg('Não foi possível gerar um usuário.')
    }
    ...
    if (data.user) {
      ...
      setUsuario(username)
      setModo('login')
      setMsg(`Cadastro criado! Seu usuário é: ${username}`)
    }
  }
  ...
  async function login() {
    ...
    const authEmail = usuarioToAuthEmail(usuario)

    if (!usuario || !senha) {
      return setMsg('Informe usuário e senha.')
    }
  ...
  if (!session) {
    return <main className="page login">
      ...
      <section className="card form">
        <h2>{modo === 'login' ? 'Entrar no bolão' : 'Primeiro cadastro'}</h2>

        <div className="loginSwitch">
          <button type="button" className={modo === 'login' ? 'activeSwitch' : ''} onClick={() => setModo('login')}>Entrar</button>
          <button type="button" className={modo === 'cadastro' ? 'activeSwitch' : ''} onClick={() => setModo('cadastro')}>Criar cadastro</button>
        </div>

        {modo === 'cadastro' && <>
          <input placeholder="Nome completo" value={nome} onChange={e => setNome(e.target.value)} />
          <input placeholder="Telefone / WhatsApp" value={whats} onChange={e => setWhats(e.target.value)} />
          <input placeholder="E-mail" value={email} onChange={e => setEmail(e.target.value)} />
          <input placeholder="Senha" type="password" value={senha} onChange={e => setSenha(e.target.value)} />
          <button onClick={signUp}>Criar cadastro</button>
          <p className="loginHint">Depois do cadastro, entre apenas com usuário e senha.</p>
        </>}
        ...
        {modo === 'login' && <>
          ...
          <button className="secondary" type="button" onClick={() => setModo('cadastro')}>Fazer primeiro cadastro</button>
        </>}
