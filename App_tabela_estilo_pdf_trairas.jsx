...
function App() {
  ...
  const groupedTableGames = useMemo(() => groupGamesByPhase(games), [games])
  ...
  return <main className="page">
    ...
    <nav className="tabs">
      ...
      <button onClick={() => setTab('tabela')} className={tab==='tabela'?'active':''}><CalendarDays/> Tabela</button>
    ...
    {tab === 'palpites' && <section className="card">
      ...
      <div className="games">
        {filteredGames.map(game => {
          ...
          return <div className={`game ${isLocked ? 'lockedGame' : ''}`} key={game.id}>
            ...
            <input disabled={isLocked} value={g.guess_home ?? ''} onChange={e => setGuess(game, 'guess_home', e.target.value)} />
            <span className="versus">x</span>
            <input disabled={isLocked} value={g.guess_away ?? ''} onChange={e => setGuess(game, 'guess_away', e.target.value)} />
            <b className="team">{flag(game.away_team)} {game.away_team}</b>
            <span className="official">{isLocked ? '🔒 ' : ''}{game.home_score === null ? 'Sem resultado' : `${game.home_score} x ${game.away_score}`} · {pts} pts</span>
          </div>
        })}
      </div>
